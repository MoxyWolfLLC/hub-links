 ...
 */
import { HUB_LINKS, hrefFor, isExternal, type HubLink, type Site } from './map.js'
...
export function segmentText(
  ...
  while (cursor < text.length) {
    ...
    let best: { index: number; length: number; value: string; link: HubLink } | null =
      null
    for (const link of links) {
      if (linked.has(link)) continue
